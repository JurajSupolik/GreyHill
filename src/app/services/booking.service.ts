...
export class ApartmentService {
  ...
  // Mock dáta - neskôr nahradíme HTTP callmi na backend
  ...
  searchApartments(filters: any): Observable<Apartment[]> {
    let filtered = [...this.mockApartments];
    ...
    if (filters.maxPrice) {
      filtered = filtered.filter(apt => apt.pricePerNight <= filters.maxPrice);
    }

    if (filters.minCapacity) {
      filtered = filtered.filter(apt => apt.capacity >= filters.minCapacity);
    }
